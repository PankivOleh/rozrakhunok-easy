    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-5">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={submit} className="space-y-3">
          <Input required value={name} onChange={(e) => setName(e.target.value)} placeholder={t("auth.name")} className="h-12 rounded-xl" />
          ...
          <Input type="password" required minLength={4} value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t("auth.password")} className="h-12 rounded-xl" />
        ...
        <Link to="/login" className="block text-center text-sm text-primary mt-4">
          {t("auth.toLogin")}
        </Link>
      </div>
    </div>
